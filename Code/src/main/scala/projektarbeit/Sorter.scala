...
/**
  * Created by patrickkoenig on 19.09.16.
  */
class Sorter(initialElementGroup: Group, pane: Pane, consoleLog: TextArea) extends Actor {
  ...
  var maxDepth: Double = Math.ceil(Math.log(initialElementGroup.children.size) / Math.log(2)) * 2 + 1

  def receive = {
    case "startSorting" => sort(initialElementGroup, 0)
    case _ => println("I don't understand you")
  }
    ...
    val timeline = new Timeline {
      ...
      keyFrames = Seq(
        ...
        at (0.2.s) {
          group.opacity -> 1.0 },
        at(1.s) {
          group.translateY -> (group.translateY() + Sorter.moveDownByPixel)
        }
      )
    }
  ...
  def createGroup(parentGroup: Group, splitList: (List[SortElement], List[SortElement]), part: EnumVal, depth: Int): Group = {
    ...
    val group = new Group() {
      ...
      translateY = parentGroup.translateY() + (if (depth > 0) Sorter.moveDownByPixel else 0)
      id = s"level-${depth + 1}"
    }
    part match {
      case Part.Left =>
  ...
  def sort(parentGroup: Group, depth: Int): Group = {
    ...
    if (elements.size > 1) {
      ...
      sequence.children.add(timelineLeft)
      ...
      sequence.children.add(timelineRight)
  ...
  def merge(leftGroup: Group, rightGroup: Group, depth: Int): Group = {
    ...
    val newYValue = Math.max(leftGroup.translateY(), rightGroup.translateY()) + Sorter.moveDownByPixel
  ...
  def relocateElement(element: SortElement, i: Int, depth: Int) = {
    ...
    val timeline = new Timeline {
      ...
      keyFrames = Seq(
        ...
        at (1.0.s) {
          element.translateY -> (element.translateY() + Sorter.moveDownByPixel)
        }
